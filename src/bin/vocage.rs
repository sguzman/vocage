...
use vocage::{load_files, PrintFormat, VocaCard, VocaSession};
...
fn main() {
    ...
    for dataset in datasets.iter_mut() {
        dataset
            .session
            .set_common_arguments(&args)
            .expect("setting common arguments");
    ...
    let mut status: String =
        "Use: q to quit, w to save, space to flip, l/→ to promote, h/← to demote, j/↓ for next"
            .to_owned();
    ...
    while !done {
        ...
        if let Some(card) = match pick_specific {
            ...
            None => {
                if ordered {
                    ...
                    for i in setindex..datasets.len() {
                        nextindex = datasets[i].next_index(
                            cardindex,
                            limit_decks.as_ref(),
                            due_only,
                            seen_only,
                    ...
                    //pick a random set
                    let setindex = if datasets.len() == 1 {
                        0
                    } else {
                        rng.gen_range(0, datasets.len())
                    };
                    //pick a random card
                    if let Some((cardindex, totalcards)) = datasets[setindex].random_index(
                        &mut rng,
                        limit_decks.as_ref(),
                        due_only,
                        seen_only,
                ...
                match c.unwrap() {
                    ...
                    Key::Char('h') | Key::Left => {
                        if card.demote(&session) {
                            status = format!(
                                "Card demoted to deck {}: {}",
                                card.deck + 1,
                                session
                                    .decks
                                    .get(card.deck as usize)
            ...
            //after a hundred attempted picks we give up
            write!(
                stdout,
                "{}{}{}{}",
                termion::clear::All,
...
pub fn draw(
    ...
    card: Option<&VocaCard>,
    session: &VocaSession,
    side: u8,
    status: &str,
    seqnr: usize,
    duecards: usize,
    minimal: Option<PrintFormat>,
) {
    ...
    if let Some(card) = card {
        let lines = card
            .fields_to_str(side, &session, true)
            .expect("printing card failed (no such side?)");
        ...
        for (i, (column, line)) in lines.into_iter().enumerate() {
            ...
            let c: color::Fg<&dyn color::Color> = match column {
